...
from model.DSConv import DCN_Conv
...
class DSCNet(nn.Module):
    def __init__(self, n_classes, n_channels, device='cuda:0', kernel_size=9, extend_scope=1.0, if_offset=True, number=16, dim=8):
        ...
        # Unet
        self.encode_in = EncoderConv(n_channels, self.number)
        ...
        self.decoder1 = DecoderConv(4*self.number, self.number)

        self.out = nn.Conv3d(self.number, n_classes, 1)
    ...
    def forward(self, x):
        ...
        x_0_3 = self.decoder1(cat([x_160_2, x_16x_2, x_16y_2, x_16z_2], dim=1))
        out = self.out(x_0_3)
