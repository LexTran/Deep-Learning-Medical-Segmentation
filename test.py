...
from utils.evaluation_metrics3D import metrics_3d, Dice_3d, over_rate, under_rate
...
args = {
    'root': './',
    'data_path': f'./data/{opts.data}',
    'pred_path': f'cor_result_160_{opts.model}',
...
def load_net():
    model = Test_Model[args['model_name']](args['n_classes']+1, 1).cuda()
...
def get_metrics(pred, gt):
    ...
    tp, fn, fp = metrics_3d(pred, gt)
    iou, dice = Dice_3d(pred, gt, gt.max()+1)
    breakpoint()
...
def model_eval(net):
    ...
    with torch.no_grad():
        ...
        for idx in range(len(images_lsts)):
            ...
            data_dict = {'img': img_path, 'label': gt_path}
            data = LoadImaged(keys=['img', 'label'], ensure_channel_first=True)(data_dict)
            image = data['img'].astype(np.int64).squeeze(0)
            label = data['label'].astype(np.int64).squeeze(0)
            # label[label > 0] = 1
            ...
            for j, patch_roi in enumerate(patch_roi_list):
                ...
                if zz1 == -1:
                    flag = 1
                    continue
                img_patch1 = image[zz1:zz1 + patch_size1, xx1:xx1 + patch_size2, yy1:yy1 + patch_size2].astype(np.float32)
            ...
            label_npy = np.flipud(label).astype(np.uint8)
            mask = np.flipud(mask)
            print('+++++++++++++')
            ...
            label_npy = np.ascontiguousarray(label_npy)
